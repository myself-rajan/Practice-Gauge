...
class DashboardReportController extends Controller
{
    ...
    public function getMonthlyCollectionData($company_id, $dateVal, $month, $CommaYear, $yearData, $filterType, $fromDate, $toDate, $compareWith, $checkedPeriod, $isLast2Year) {
        ...
        if($filterType == 7 || $filterType == 8 || $filterType == 9) {
            $yearData = $this->fetchFilterData($filterType, $yearData, $CommaYear, $fromDate, $toDate);
        }
        else {
            $filterArr = $this->fetchFilterData($filterType, $yearData, $CommaYear, $fromDate, $toDate);
        }
        ...
        else if($filterType == 5 || $filterType == 6 || $month > 0) {
            $fetchAccountData->whereIn(\DB::raw('MONTH(accounts_data.date)') , $filterArr);
        }
        /*else if($month > 0){
    ...
    public function getoperationalExpensesReportData($company_id, $dateVal, $month, $CommaYear, $yearArr, $filterType, $fromDate, $toDate, $compareWith, $checkedPeriod, $isLast2Year) {
        ...
        $yearArr = $this->fetchFilterData($filterType, $yearArr, $CommaYear, $fromDate, $toDate);
    ...
    public function getNetCollectionData($accountTypeArr=[], $yearArr=[], $month, $filterType, $fromDate, $toDate, $compareWith, $checkedPeriod, $isLast2Year)
    {
        ...
        $filterArr = $this->fetchFilterData($filterType, $yearArr='', $CommaYear='', $fromDate='', $toDate='');
        ...
        if($filterType == 5 || $filterType == 6 || $month > 0) {
            $fetchAccountData->whereIn(\DB::raw('MONTH(accounts_data.date)') , $filterArr);
        }
    ...
    public function fetchFilterData($filterType, $yearArr, $CommaYear, $fromDate, $toDate) {

        $filterArr = [];
        if($filterType == 3) {
           $filterArr [] = ltrim(date('m'), '0');

        } 
        else if($filterType == 4) {
           $filterArr [] = ltrim(date('m') - 1, '0');
        }
            ...
            if($filterType == 5) {
                ...
                $gnMonth = date('n', time()) - 3;
            }

            if($gnMonth == 1 || $gnMonth == 2 || $gnMonth == 3) {
                $start = 1;
                $filterArr = [1,2,3];
            }
            else if($gnMonth == 4 || $gnMonth == 5 || $gnMonth == 6) {
                $start = 4;
                $filterArr = [4,5,6];
            }
            else if($gnMonth == 7 || $gnMonth == 8 || $gnMonth == 9) {
                $start = 7;
                $filterArr = [7,8,9];
            }
            else if($gnMonth == 10 || $gnMonth == 11 || $gnMonth == 12) {
                $start = 10;
                $filterArr = [10,11,12];
            }
        }
    ...
    public function getNetOperatingIncomeData($yearArr=[], $month, $filterType, $fromDate, $toDate, $isNetOperating='', $accountType = '', $partCategory = '', $compareWith, $checkedPeriod, $isLast2Year)
    {
        ...
        if($filterType == 9) {
            ...
            $fetchAccountData->whereIn(\DB::raw('MONTH(ad.date)') , $filterArr);
        }
        /*else if($month > 0){
     ...
     /*public function generatePDF(Request $request)
    {
        ...
        $fetchData = $this->getMonthly($company_id, $dateVal, $month, $CommaYear, $yearData, $filterType, $fromDate, $toDate, $compareWith, $isLast2Year='');

        $netBarChartData = $this->getGross($company_id, $dateVal, $month, $CommaYear, $yearData, $filterType, $fromDate, $toDate, $compareWith, $isLast2Year='');

        $categoryBarData = $this->getoperational($company_id, $dateVal, $month, $CommaYear, $yearData, $filterType, $fromDate, $toDate, $compareWith, $isLast2Year='');
