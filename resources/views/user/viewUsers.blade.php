              ...
              @if(count($users) > 0)
                @foreach($users as $row)
                  <tr>
                    ...
                    <td>{{ $row->email }}</td>
                    @foreach($roleUser as $rowRole)
                      @if($rowRole['id'] == $row->role_id)
                        <td>{{ $rowRole['name'] }}</td>
                      @endif
                    @endforeach
                    @if($row->active == 1)
                    ...
                    <td class="text-right">
                      <a class="btn btn-sm btn-light rounded-pill text-primary userValId" data-toggle="modal" data-target="#mAddUser" data-id="{{$row->id}}"><i class="fas fa-pencil-alt" style="cursor: pointer"></i></a>
                    </td>
                  </tr>
                @endforeach
                  ...
                  <div class="row">
                    <!-- <div class="col-sm-4 col-md-3 col-xxl-2">
                      <div class="position-relative">
                        <img src="{{asset('/img/users/Oliver_Queen.jpg')}}" class="img-fluid img-thumbnail bg-white mb-2 w-100" id="showProfilePicEdit"
                          alt="Profile Picture" />
